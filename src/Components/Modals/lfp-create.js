...
class LFPCreateModal extends Component {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const gameValidation = validateGameConfig("lfp", game);
      ...
      const rawContent = modalHandler.extractContent(interaction, "lfp", game);
      const { content, validationResult } = validateAndSanitizeContent(rawContent, "lfp", game);
    ...
    // 💾 Save request with timeout handling
    const now = new Date();
    ...
    let req;
    try {
      req = await timeoutHandler.handleDatabaseOperation(
        () => LFRequest.create({
          userId: user.id,
          guildId: guild.id,
          type: "LFP",
        ...
        "LFP request creation"
      );
    } catch (error) {
      // Handle duplicate key error (E11000) - database constraint prevents duplicate pending requests
      if (error.code === 11000) {
        logger.info(`Duplicate key error for user ${user.id} creating LFP request for ${game} - user already has pending request`);
    ...
    const msg = await timeoutHandler.handleMessageSend(
      reviewChannel, 
      { embeds: [reviewEmbed], components: [row] }
    );
    
    if (!msg) {
      logger.error(`Failed to send review message for request ${req._id}`);
      
      // Rollback: Delete the created request since message sending failed
      try {
        await timeoutHandler.handleDatabaseOperation(
          () => LFRequest.findByIdAndDelete(req._id),
          "LFP request rollback after message send failure"
        );
        logger.info(`Successfully rolled back LFP request ${req._id} after message send failure`);
      } catch (rollbackError) {
        logger.error(`Failed to rollback LFP request ${req._id}: ${rollbackError.message}`);
    ...
    await timeoutHandler.handleDatabaseOperation(
      () => req.save(),
      "LFP request message ID update"
    );

    // Log the action
    await logLFAction(interaction.client, config, 'create', req, user);

    // ✅ Reply to user with timeout handling
    const replySuccess = await timeoutHandler.handleInteractionReply(
      interaction,
      { 
    ...
    if (!replySuccess) {
      logger.error(`Failed to reply to user ${user.id} for LFP request ${req._id}`);
    }
    ...
    try {
      ...
      logger.error(`Error in LFP create modal for user ${user.id}: ${error.message}`);
      ...
      if (error.code === 11000) {
        errorMessage = "You already have a pending LFP request for this game. Please cancel it first using `/requests cancel`.";
